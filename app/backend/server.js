...
router.route('POST', '/api/chat/download-test', (req, res) => {
    console.log('📥 Download test endpoint called');
...
server.listen(PORT, async () => {
    ...
    console.log(`✨ Features: Modular Architecture, Auto-detection, Auto-start, Chat Storage, Privacy-First`);
    console.log(`💾 Storage location: ${chatStorage.dataPath}`);

    // Show initial storage statistics
    const stats = chatStorage.getStorageStats();
    if (stats.success) {
        console.log(`📊 Storage stats: ${stats.stats.totalChats} chats, ${stats.stats.totalMessages} messages, ${stats.stats.totalSizeMB}MB`);
        logger.app('info', 'Storage stats loaded', stats.stats);
    }

    // Show refactored architecture
    console.log(`🏗️ Architecture: ${router.routes.length} API routes registered`);
    console.log(`📦 Controllers: Chat, Model, Ollama, MCP, Log`);
    ...
    setTimeout(async () => {
        ...
        if (result) {
            console.log(`✅ Ollama started successfully - http://localhost:11434`);
            logger.app('info', 'Ollama started successfully');
        } else {
            console.log(`⚠️ Failed to start Ollama - check if it's installed`);
            logger.app('warn', 'Failed to auto-start Ollama');
        }

        // Open browser automatically after initialization
        console.log(`🌐 Opening Ollama Easy GUI in your default browser...`);
        setTimeout(() => {
